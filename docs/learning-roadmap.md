...
当前基础生产闭环已经完成，后续学习进入高并发、多租户、高可用和高合规场景。实现时仍优先保持模块化单体，只有在状态边界、通信边界和独立扩缩容需求被验证后，才评估服务拆分。

下一阶段先从多租户与数据隔离开始，因为它会同时影响数据模型、权限、Repository、缓存键、异步消息和测试边界：

- 增加 Tenant/Organization 与成员归属模型。
- 建立请求级租户上下文，并让业务查询强制携带租户范围。
- 使用复合唯一约束、索引和 E2E 测试防止跨租户数据泄露。
- 推演管理员、停用账号、批量接口和后台任务中的租户边界。
- 暂不引入微服务或 PostgreSQL RLS，先明确应用层隔离责任，再比较 RLS 的收益和代价。

后续能力按以下顺序递进：

1. 多租户与数据隔离：租户上下文、权限传播、数据约束和隔离验证。
2. 高并发与分布式协作：Redis、分布式限流、队列、租约恢复、背压和压测。
3. 高可用与恢复：多实例、就绪探针、滚动发布、备份恢复、故障演练和 SLO。
4. 高合规与治理：审计、敏感数据、密钥轮换、数据留存、权限审查和安全发布。
...
状态：规划中。该迭代是复杂生产系统能力的第一步，用于把当前项目成员级授权提升为租户级数据边界，并验证隔离规则在请求、后台任务和持久化层都不会被绕过。
