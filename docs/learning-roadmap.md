...
本项目不把“会使用 NestJS、数据库或中间件”作为最终目标，而是训练在不可靠的计算、网络和存储环境中持续维护业务状态正确性的能力。

配套的[后端系统思维课程](https://github.com/leidaibo-lab/backend-system-thinking)负责建立问题意识、概念模型和故障推演；本项目负责将这些判断落成模块、数据约束、测试、部署和运行证据。两者的关系是“先理解为什么，再实现并证明怎么做”，不是两套互相独立的学习路线。
...
这些问题需要同时从六个纵向层次和五条横向主线检查：

- 纵向层次：领域与契约、应用编排、数据与状态、通信与分布式、运行时与资源、运行与治理。
- 横向主线：正确性、可靠性、效率、信任和演进。

### 系统思维落地闭环

后续每个学习增量均遵循以下闭环：

1. 先用业务场景描述条件变化，而不是先选择技术工具。
2. 明确边界、权威事实、不变量、失败状态和可接受取舍。
3. 使用 OpenSpec 写出可验收的行为、设计和任务。
4. 在 NestJS 主线中实现一条完整纵向链路。
5. 通过单元、集成、E2E、并发或故障注入测试验证结论。
6. 记录工具解决了什么问题、引入了什么新状态和故障。
7. 复盘系统如何观察、恢复、发布和长期演进。

完成标准不再是“功能成功返回”，而是能够解释系统在并发、重复、乱序、超时、迁移和发布变化下为什么仍然正确。
...
基础生产闭环和多租户数据隔离已经完成，后续学习进入高并发、高可用和高合规场景。实现时仍优先保持模块化单体，只有在状态边界、通信边界和独立扩缩容需求被验证后，才评估服务拆分。

下一阶段进入高并发与分布式协作，重点研究 Redis、分布式限流、队列、租约恢复、背压和压测，并继续验证租户上下文在缓存键、异步消息和后台任务中的传播边界。

后续能力按以下顺序递进：

1. 高并发与分布式协作：Redis、分布式限流、队列、租约恢复、背压和压测。
2. 高可用与恢复：多实例、就绪探针、滚动发布、备份恢复、故障演练和 SLO。
3. 高合规与治理：审计、敏感数据、密钥轮换、数据留存、权限审查和安全发布。

每个阶段都必须说明非目标、故障模型、容量假设、验证证据和运维边界，不以引入中间件数量作为完成标准。
...
状态：已完成，变更为 `add-multi-tenant-isolation`。该迭代是复杂生产系统能力的第一步，把当前项目成员级授权提升为租户级数据边界，并验证隔离规则在请求、后台任务和持久化层都不会被绕过。

- 增加租户归属、成员关系和租户级角色模型。
- 将租户上下文贯穿 Guard、Service、Repository 和异步通知载荷。
- 为租户范围查询增加复合约束、索引和跨租户越权测试。
- 通过场景推演比较应用层过滤、Repository 封装和数据库 RLS 的责任边界。
- 不在本迭代实现微服务拆分、完整计费系统或跨租户数据共享。

本次增量已完成：

- 新增租户和租户成员模型；注册用户自动获得默认租户，也可创建并查询自己的租户。
- 使用 `x-tenant-id` 建立请求级租户上下文；单租户请求兼容自动选择，多租户请求必须显式选择。
- 项目、任务 Repository、任务事件、通知 outbox 和通知查询均携带租户范围。
- 迁移已为历史用户和项目回填租户归属，并通过 E2E、集成测试验证跨租户资源不可读写。
