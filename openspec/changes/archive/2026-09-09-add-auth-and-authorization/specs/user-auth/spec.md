...
系统 MUST 校验邮箱和密码，凭据正确时签发包含用户标识和过期时间的 JWT；凭据错误时 MUST 使用统一的 HTTP 401 响应，不泄露邮箱是否存在。

#### Scenario: 使用正确凭据登录

- **WHEN** 已注册用户向 `POST /auth/login` 提交正确邮箱和密码
- **THEN** 系统返回 HTTP 200 和可验证的 JWT accessToken

#### Scenario: 使用错误凭据登录

- **WHEN** 客户端提交不存在的邮箱或错误密码
- **THEN** 系统返回 HTTP 401，且不返回 accessToken
